...
def getClassStudents(request, id_class):
    ...
    try:
        ...
        if (clas) and (classes.filter(id_class=id_class)):
            ...
            #students=Student.objects.filter(kaid_student__in=Student_Class.objects.filter(id_class_id=id_class).values('kaid_student'))
            incorrect = Skill_Attempt.objects.filter((Q(kaid_student__in=students)&Q(correct=False)&Q(skipped=False))|(Q(kaid_student__in=students)&Q(correct=False)&Q(skipped=True)&Q(count_attempts__gte=2))).values('kaid_student_id').annotate(incorrect=Count('kaid_student_id'))   
