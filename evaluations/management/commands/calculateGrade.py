            ...
            for grade in grades:
                if grade.evaluated == False:
                    ...
                    for sk_lg in skills_log:
                        ...
                        try:#id04
                            incorrect = Skill_Attempt.objects.filter(kaid_student=grade.kaid_student,id_skill_name_id=sk_lg.id_skill_name,correct=False,(Q(skipped=False)|Q(skipped=true)&Q(task_type__startswith='mastery.')),date__range=(assesment.start_date,assesment.end_date)).values('kaid_student_id').annotate(incorrect=Count('kaid_student_id'))
